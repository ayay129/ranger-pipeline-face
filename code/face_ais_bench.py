...
class FaceModelAISBench:
    ...
    def _preprocess_kps(self, face: np.ndarray) -> np.ndarray:
        ...
        for bb in bboxes:
            ...
            if self.kps_sess is not None:
                if crop.size != 0:
                    ...
                    sel_align = np.array([pts106[8], pts106[36], pts106[30], pts106[54], pts106[48]], dtype=np.float32)
    ...
    def get_gender_age_race(self, aligned_face: np.ndarray):
        ...
        for a in arrs:
            ...
            if v.ndim == 1 and v.shape[0] == 3:
                ...
                race = ["Asian", "White", "Black", "Indian", "Other"][int(np.argmax(v))]
